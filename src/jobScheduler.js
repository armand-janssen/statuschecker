...
function schedule(myJob) {
  ...
  const scheduledJob = scheduler.scheduleJob(job.pattern, async (fireDate) => {
    ...
    try {
      ...
      if (job.hasAlreadyFailed()) {
        processAlreadyFailed(now, job, resultFromCheck.httpStatus, error.message);
      } else {
        processFailedFirstTime(now, job, resultFromCheck.httpStatus, `${error.message}`); // - ${resultFromCheck.response}`);
      }
    } finally {
      job.checked(now);
    }
